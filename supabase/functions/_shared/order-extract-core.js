...
export function buildOrderExtractPrompt() {
  return [
    ...
    '10. 若图纸同一字段区域本身写有多个值（例如同时出现两个订单号 D26309、D26308）：不删除、不合并、不猜测它们之间的关系或哪个有效；字段值填其中最明确、最清晰的那一个，并在 evidence 里写明「图上存在多个候选（原文）」。无法确定哪个最明确 → 填 "UNKNOWN"。',
    '',
    '【6 个字段】',
    '- customer_name（客户名称）：客户的代号/简称，通常位于图纸右下角「生产编号」那一格的旁边一格（紧邻）。它是一个短代号，形态多为字母或字母+少量数字（通常比「订单号码」短）。图上通常没有"客户名称"四个字的文字标签，不要去找"客户名称"字样。排除规则：不要取以 R/SA/SH 开头的编号（那是生产编号），也不要取左上角「订单号码」格里的内容，更不要取「壳号」。只有明确看到这个短代号才填；找不到、看不清、或无法确认哪一个是客户代号 → 填 "UNKNOWN"，禁止按客户习惯或常见代号猜测。',
    ...
    '- base_texture（底纹处理）：枚举 "无底纹"/"太阳纹"/"直线纹"/"其他"。默认填 "UNKNOWN"。底纹必须来源于图中文字，按下面三步判断：① 文字明确且单一地写出底纹类型 → 按映射填写（"无纹/光面/素面/光底/无底纹"→"无底纹"；"车CD纹/车唱片纹/多种组合"→"其他"；"太阳纹"→"太阳纹"；"直线纹"→"直线纹"）；② 图中没有任何底纹文字 → "UNKNOWN"；③ 底纹文字模糊/看不清/列出多档选项（如 A/B/C）/无法确定 → "UNKNOWN"，让用户自己填。【最高优先级】即使表盘视觉上呈现放射纹、同心圆、太阳纹或类似效果，只要没有文字写"太阳纹"，禁止输出"太阳纹"——视觉纹理不作为任何判断依据，只以文字为准。',
    ...
    '',
    '【示例】',
    '示例1（正例）：图中文字明确写着「太阳纹」→ base_texture 填 "太阳纹"。',
    '示例2（反例）：表盘视觉上呈放射状/类似太阳纹效果，但图中没有任何底纹文字 → base_texture 必须填 "UNKNOWN"，绝不允许因为看到放射纹就填 "太阳纹"。',
    '',
    '【输出格式】',
